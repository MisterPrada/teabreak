<?php
	...
	switch($op){
		case 'save': // save the item
			$new_art=true;
			if($id>0){
				$SQL = sprintf("UPDATE articles SET title=%s,date=FROM_UNIXTIME(%s), content=%s WHERE id=%s",quote_smart($_POST['title']),quote_smart($_POST['date']),quote_smart($_POST['text']),quote_smart($id)); 
				$new_art=false;
			} else {
				// insert new items
				$SQL = sprintf("INSERT INTO articles(title,date,short_descr,content,type) VALUES(%s,FROM_UNIXTIME(%s),NULL,%s,'blog')",quote_smart($_POST['title']),quote_smart($_POST['date']),quote_smart($_POST['text'])); 
			}
			...
			if($new_art){ // send notifications
				...
				while ($db_field = mysql_fetch_assoc($result)) {
					...
					$body = 'На сайте <a href="http://www.teabreak.by">"Чайная пауза"</a> - обновление. <br>'.$_POST['title'].'<br>'.substr($_POST['text'],0,400)."... <a href='http://www.teabreak.by/blog.php?art_id=".$id."'>Читать далее...</a>";
	...
	// get item details
	$SQL = sprintf("SELECT id,date,title,content FROM articles WHERE id=%s",quote_smart($id) );				
	$result = mysql_query($SQL);
	...
	$smarty->assign('DATE2',strtotime($date));
	$smarty->assign('TEXT',$content);
	$smarty->assign('LINKS',$selectTree);
	$smarty->assign('RELATED_CATEGORIES',$rel_cat);

	$smarty->assign('BLOG_MENU',true);
	$smarty->assign('CONTENT',$smarty->fetch("admin/blogitem.tpl"));
