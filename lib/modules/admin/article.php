<?php
	...
	switch($op){
		case 'save': // save the item
			...
			if($id>0){
				...
				// insert new items
				$SQL = sprintf("INSERT INTO articles(title,date,short_descr,content,type) VALUES(%s,FROM_UNIXTIME(%s),%s,%s,'article')",quote_smart($_POST['title']),quote_smart($_POST['date']),quote_smart($_POST['short_descr']),quote_smart($_POST['text'])); 
			}
			...
			if($new_art){ // send notifications
				...
				while ($db_field = mysql_fetch_assoc($result)) {
					...
					$body = 'На сайте <a href="http://www.teabreak.by">"Чайная пауза"</a> - обновление. <br>'.$_POST['title'].'<br>'.substr($_POST['text'],0,400)."... <a href='http://www.teabreak.by/articles.php?art_id=".$id."'>Читать далее...</a>";
	...
	$smarty->assign('ARTICLES_MENU',true);
	$smarty->assign('CONTENT',$smarty->fetch("admin/article.tpl"));
